...
#SBATCH --mem=32G
#SBATCH -c 4
#SBATCH -o /nfs/hpc/share/zhanyaol/claude-code/logs/val_code_fix2_%j.out
#SBATCH -e /nfs/hpc/share/zhanyaol/claude-code/logs/val_code_fix2_%j.err

echo "=== Validate code fix: EvoPool (fixed) on MBPP 50 ==="
...
echo "=== EvoPool full (with test case fix) on MBPP 50 ==="
...
echo "=== noCoDream (with test case fix) on MBPP 50 ==="
mkdir -p results/val_nocd_mbpp50
python -u run_experiment.py \
    ...
    --backbone_llm "qwen3-8b" \
    --seed 42 \
    --output_dir results/val_nocd_mbpp50/
