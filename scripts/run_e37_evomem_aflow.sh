...
#SBATCH --mem=32G
#SBATCH -c 4
#SBATCH -o /nfs/hpc/share/zhanyaol/claude-code/logs/e37_evomem_aflow_%j.out
#SBATCH -e /nfs/hpc/share/zhanyaol/claude-code/logs/e37_evomem_aflow_%j.err

# E37: EvoMem baseline on AFlow-Stream
# EvoMem: individual self-reflection memory (Reflexion-style), NO cross-agent CoDream
# Tests whether individual memory evolution alone accounts for EvoPool's gains.
# If EvoPool >> EvoMem → collective CoDream is critical, not just individual memory.
echo "=== E37: EvoMem (individual evolving memory) on AFlow-Stream ==="
...
python -u run_experiment.py \
    ...
    --seed 42 \
    --output_dir results/e37/

echo "=== Done: EvoMem AFlow | $(date) ==="
