#!/bin/bash
#SBATCH -J evopool_e53_evomem_aime_think2
#SBATCH -A hw-grp
#SBATCH -p hw-grp
#SBATCH --gres=gpu:0
#SBATCH -t 8:00:00
#SBATCH --mem=32G
#SBATCH -c 4
#SBATCH -o /nfs/hpc/share/zhanyaol/claude-code/logs/e53_evomem_aime_think2_%j.out
#SBATCH -e /nfs/hpc/share/zhanyaol/claude-code/logs/e53_evomem_aime_think2_%j.err

# E53: EvoMem on Hard Math Stream (thinking=True)
# Thinking mode ENABLED — required for AIME (long reasoning chains)
echo "=== E53: EvoMem on Hard Math Stream (thinking=True) ==="
...
python -u run_experiment.py \
    ...
    --output_dir results/e53/

echo "=== Done: EvoMem on Hard Math Stream (thinking=True) | $(date) ==="
