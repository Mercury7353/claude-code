#!/bin/bash
#SBATCH -J evopool_e43_memcollab_aime_think
#SBATCH -A hw-grp
#SBATCH -p hw-grp
#SBATCH --gres=gpu:0
#SBATCH -t 8:00:00
#SBATCH --mem=32G
#SBATCH -c 4
#SBATCH -o /nfs/hpc/share/zhanyaol/claude-code/logs/e43_memcollab_aime_think_%j.out
#SBATCH -e /nfs/hpc/share/zhanyaol/claude-code/logs/e43_memcollab_aime_think_%j.err

# E43: MemCollab on Hard Math Stream (thinking=True)
# Thinking mode ENABLED — required for AIME (long reasoning chains)
echo "=== E43: MemCollab on Hard Math Stream (thinking=True) ==="
...
python -u run_experiment.py \
    ...
    --output_dir results/e43/

echo "=== Done: MemCollab on Hard Math Stream (thinking=True) | $(date) ==="
